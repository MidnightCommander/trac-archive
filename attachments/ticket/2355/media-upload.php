...
function media_upload_nextgen_form($errors) {
	...
	// Get the images
	if ( $galleryID != 0 )
		$picarray = $wpdb->get_col("SELECT pid FROM $wpdb->nggpictures WHERE galleryid = '$galleryID' AND exclude != 1 ORDER BY {$ngg->options['galSort']} {$ngg->options['galSortDir']} LIMIT $start, $ngg_max_images_per_page "); //BB Dev	
...
<!-- BB Dev -->		  <button style="float:right;margin-right:10px;margin-top:8px;" type="submit" class="button" value="1" name="send[<?php echo $picid ?>]"><?php echo attribute_escape( __('Insert into Post') ); ?></button>
